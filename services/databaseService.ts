...
export const addMessage = async (supabase: SupabaseClient, message: Omit<Message, 'id' | 'created_at'>): Promise<Message> => {
    ...
    // Fix: Map camelCase to snake_case for DB insertion and remove camelCase key to avoid 400 errors
    if ('groundingMetadata' in messageToInsert) {
        if (messageToInsert.groundingMetadata) {
            messageToInsert.grounding_metadata = messageToInsert.groundingMetadata;
        }
        // CRITICAL: Always delete the camelCase key. If the column doesn't exist in DB, 
        // keeping this key (even if undefined/null) might cause a 400 error depending on PostgREST config.
...
export const updateMessage = async (supabase: SupabaseClient, messageId: string, updates: Partial<Message>): Promise<Message> => {
    ...
    // Fix: Map camelCase to snake_case for DB update
    if ('groundingMetadata' in updatesToInsert) {
        if (updatesToInsert.groundingMetadata) {
